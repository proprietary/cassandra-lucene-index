...
/**
 * Acceptance tests for range search using doc values.
 *
 * @author Andres de la Pena <adelapena@stratio.com>
 */

public class RangeSearchWithDocValuesTest extends AbstractSearchTest {

    private static RangeCondition range(String field) {
        return new RangeCondition(field).docValues(true);
    }
    ...
    @Test
    public void testRangeTextField() {
        filter(range("text_1")).check(InvalidQueryException.class, UNSUPPORTED_DOC_VALUES);
    }
    ...
    public void testRangeListField1() {
        filter(range("string_list").lower("a").upper("z")).check(5);
    }

    @Test
    public void testRangeListField2() {
        filter(range("string_list").lower("a1").upper("z9")).check(5);
    }

    @Test
    public void testRangeListField3() {
        filter(range("string_list").lower("a2").upper("l1")).check(0);
    }

    @Test
    public void testRangeSetField1() {
        filter(range("string_set").lower("a").upper("z")).check(5);
    }

    @Test
    public void testRangeSetField2() {
        filter(range("string_set").lower("a1").upper("z9")).check(5);
    }

    @Test
    public void testRangeSetField3() {
        filter(range("string_set").lower("a1").upper("z1")).check(5);
    }
