...
public class CassandraUtils {
    ...
    public ResultSet execute(String query, Object... args) {
        return execute(CassandraConfig.FETCH, query, args);
    }
    ...
    public CassandraUtils createKeyspace() {
        execute(new StringBuilder().append("CREATE KEYSPACE ")
                    ...
                    .append(CassandraConfig.REPLICATION)
                    .append("' };"));
        return this;
    }
    ...
    public CassandraUtils createIndex() {
        Index index = index(keyspace, table, indexName).column(indexColumn)
            .refreshSeconds(CassandraConfig.REFRESH)
            .indexingThreads(CassandraConfig.THREADS)
            .partitioner(partitioner)
            .sparse(sparse);
    ...
    public List<Row> searchWithPreparedStatement(Search search) {
        String query = useNewQuerySyntax
            ? String.format("SELECT * FROM %s WHERE expr(%s,?) LIMIT %d", qualifiedTable, indexName, CassandraConfig.LIMIT)
            : String.format("SELECT * FROM %s WHERE %s = ? LIMIT %d", qualifiedTable, indexColumn, CassandraConfig.LIMIT);
    ...
    public CassandraUtils flush() {
        ...
        CassandraConnection.invokeJMXMethod("org.apache.cassandra.db:type=StorageService",
                                            "forceKeyspaceFlush",
    ...
    public CassandraUtils refresh() {
        ...
        CassandraConnection.invokeJMXMethod(indexBean, "refresh", new Object[]{}, new String[]{});
        return this;
    }

    public CassandraUtils commit() {
        logger.debug("JMX: Commit");
        CassandraConnection.invokeJMXMethod(indexBean, "commit", new Object[]{}, new String[]{});
        return this;
    }
    ...
    public CassandraUtils checkNumDocsInIndex(Integer expectedNumDocs) {
        List<Long> numDocsInEachNode = CassandraConnection.getJMXAttribute(indexBean, "NumDocs");
        Long totalNumDocs = numDocsInEachNode.stream().reduce(0L, (l, r) -> l + r) / (long) CassandraConfig.REPLICATION;
    ...
    public CassandraUtils checkNumDeletedDocsInIndex(Integer expectedNumDeletedDocs) {
        List<Long> numDeletedDocsInEachNode = CassandraConnection.getJMXAttribute(indexBean, "NumDeletedDocs");
        Long totalNumDocs = numDeletedDocsInEachNode.stream().reduce(0L, (l, r) -> l + r) / (long) CassandraConfig.REPLICATION;
    ...
    private boolean isIndexBuilt() {
        ...
        List<List<String>> builtIndexes = CassandraConnection.getJMXAttribute(bean, "BuiltIndexes");
